#!/usr/bin/env python3
"""
Codex-powered autonomous code review for GitHub PRs.

Reads PR context via GITHUB_EVENT_PATH, loads review guidelines from
prompts/code-review.md, asks Codex to produce JSON findings, and posts them
as inline review comments via the GitHub API.

Requires the codex_native extension to be installed in the environment.
The workflow builds it with maturin before running this script.
"""
...
# Local codex library (imported from repository), native extension must be installed.
from codex.config import ApprovalPolicy, CodexConfig
    ...
    *,
    ...
    try:
        with urlopen(req) as resp:  # nosec - GitHub API
            ...
            return raw
    except HTTPError as e:  # surface error payload for debugging
        try:
            raw = e.read()
...
def _parse_valid_head_lines_from_patch(patch: str) -> set[int]:
    """Return the set of HEAD-side line numbers present in a unified diff patch.

    We treat context (' ' prefix) and additions ('+') as valid inline-anchorable
    lines on the RIGHT side. Deletions ('-') are not valid on the head side.
    """
    valid: set[int] = set()
    i_old = i_new = 0
    for line in patch.splitlines():
        if line.startswith("@@"):
            # Example: @@ -12,7 +12,9 @@ optional section
            try:
                header = line.split("@@")[1].strip()
            except Exception:
                header = line
            try:
                minus, plus = header.split(" ")[0:2]
            except Exception:
                # Fallback: find + segment
                parts = header.split()
                minus = parts[0]
                plus = parts[1] if len(parts) > 1 else "+0,0"
            # Parse -a,b and +c,d
            try:
                plus_nums = plus[1:].split(",")
                i_new = int(plus_nums[0]) - 1  # will pre-increment below
                minus_nums = minus[1:].split(",")
                i_old = int(minus_nums[0]) - 1
            except Exception:
                i_new = i_old = 0
            continue
...
def _compute_position_from_patch(patch: str, target_head_line: int) -> int | None:
    """Compute the diff `position` for a given head-side line number.

    Position counts non-header lines starting from 1 below the first hunk header,
    continues across hunks to the end of the file diff. We return the position of
    the first patch line whose head-side line equals `target_head_line` for tags
    ' ' (context) or '+' (addition). Deletions ('-') cannot be targeted.
    """
    pos = 0
    i_old = i_new = 0
    in_hunk = False
    for line in patch.splitlines():
        ...
        if not line:
            continue
        tag = line[0]
        # Count position for every non-header line
        pos += 1
        if tag == " ":
            i_old += 1
            i_new += 1
            ...
            # informational line; does not affect counters
            pass
    return None
...
def _load_guidelines() -> str:
    ...
    path = Path("prompts/code-review.md")
    if not path.exists():
        print("prompts/code-review.md not found", file=sys.stderr)
        sys.exit(1)
    ...
    if provider == "openai":
        if not os.environ.get("OPENAI_API_KEY"):
            print(
                ...
                "Set it as a repository secret and expose to the job.",
                file=sys.stderr,
            )
            sys.exit(2)
...
def _run_codex(prompt: str) -> str:
    ...
    for item in stream:
        # Each item is a dict envelope { id, msg: { type, ... } }
    ...
    *,
    ...
    return cast(
        ...
        _github_api(
            ...
            token=token,
            data=payload,
            # Required for line/side requests per GitHub API (comfort-fade preview)
...
def main() -> None:
    ...
    pr_number = int(pr_evt["number"]) if "number" in pr_evt else int(event.get("number", 0))
    # Fetch fresh PR to ensure up-to-date head SHA for comment anchoring
    ...
    # Load guidelines and compose the single large prompt
    ...
    for fch in changed[:10]:  # cap to avoid log spam
        _dbg(
            2,
    ...
    except Exception as e:  # pragma: no cover - robust reporting
    ...
    except HTTPError as e:  # pragma: no cover - external API
        print(f"Failed to post review summary: {e}", file=sys.stderr)

    # Build fast lookup of valid commentable lines per changed file
    ...
    for fch in changed:
        if fch.patch:
            ...
            # Precompute positions for valid head lines
            ...
            _dbg(
                2,
                f"Anchor map ready for {fch.filename}: valid_lines={len(valid_lines_by_path[fch.filename])} positions={len(pos_map)}",
            )

    # Post each finding as an inline (or fallback file) comment
    for f in findings:
        ...
        # Map absolute path back to repo-relative path
        try:
            rel_path = str(Path(abs_path).resolve().relative_to(repo_root))
        except Exception:
            # If not under root, assume the path is relative already
            rel_path = abs_path.lstrip("./")

        # Decide whether we can anchor to a valid diff line
        valid_lines_by_path.get(rel_path)
        ...
        try:
            if can_anchor and pos is not None:
                ...
                # Fallback: file-level comment when line is not part of the diff
            ...
            # Slight delay to avoid secondary rate limiting
            time.sleep(0.2)
        except HTTPError as e:  # pragma: no cover - external API
