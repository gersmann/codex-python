...
def main() -> int:
    ...
    # Add file-level directives for generated-code tooling noise and forward refs.
    lines = s.splitlines()
    ...
    if "# ruff: noqa: F821" not in lines[:8]:
        ...
        directive_insert_at += 1
    if "# mypy: ignore-errors" not in lines[:8]:
        lines.insert(directive_insert_at, "# mypy: ignore-errors")
    s = "\n".join(lines) + ("\n" if not s.endswith("\n") else "")
